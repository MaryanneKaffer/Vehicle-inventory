...
export default function VehiclesList({ vehicles, loading, message, apiPages, setPage, screen, mbDelete, mbView, logged }: {
    setPage: (page: number) => void, vehicles: Vehicle[], loading: boolean, message: string, apiPages: number, screen: string,
    mbDelete: boolean, mbView: boolean, logged: User | null
}) {
    return (
            ...
            {!message && !loading &&
                ...
                grid  gap-2 sm:gap-4 justify-between`}>
                    {vehicles.map((vehicle, i) => (
                        <VehicleCard key={vehicle.name} setPage={setPage} mbDelete={mbDelete} mbView={mbView} i={i} vehicle={vehicle} logged={logged} />
                    ))}
                </div>
            }
        </div>
